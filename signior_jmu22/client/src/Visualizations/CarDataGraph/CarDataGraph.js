...
class CarDataGraph extends Component {
  constructor(props) {
    super(props);
    this.state = {
      graphData: [], 
  ...
  componentWillReceiveProps(nextProps) {
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      let carData = currRow['car_data'];
      for (let i = 0; i < carData.length; i++) {
        let newRow = {};
        let yearData = carData[i];
  ...
  render() {
    ...
    if (countries.length === 0) {
      linegraph = 
        <div>
          <h5 style={{textAlign:"center"}}>Number of Cars per Year (By Country)</h5>
          <LineChart
            width={1250}
            height={500}
            data={data}
